...
#AdamOptimizer
#train_step_adam = tf.train.AdamOptimizer(1e-4).minimize(cross_entropy)

#AdagradOptimizer
train_step_adagrad = tf.train.AdagradOptimizer(1e-4).minimize(cross_entropy)
...
#     summary_adam, _ = sess.run([merged, train_step_adam], feed_dict={x: batch_adam[0], y_: batch_adam[1], keep_prob: 0.5})
...
for i in range(500):
    batch_adagrad = mnist.train.next_batch(50)
    summary, _ = sess.run([merged, train_step_adagrad], feed_dict={x: batch_adagrad[0], y_: batch_adagrad[1], keep_prob: 0.5})
    trainwriter.add_summary(summary, i)
    if i % 100 == 0:
        train_accuracy_adagrad = accuracy.eval(feed_dict={x: batch_adagrad[0], y_: batch_adagrad[1], keep_prob: 1.0})
        print("step %d, training accuracy %g" % (i, train_accuracy_adagrad))
